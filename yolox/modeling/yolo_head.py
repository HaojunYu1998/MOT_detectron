...
class YOLOXHead(nn.Module):
    ...
    def __init__(self, cfg, input_shape):
        ...
        self.obj_loss = sigmoid_focal_loss_jit
    ...
    def forward(self, xin, labels=None, imgs=None):
        xin = [xin[f] for f in self.in_features]
        outputs = []
        origin_preds = []
        x_shifts = []
        y_shifts = []
        ...
        if self.training:
            ...
            return self.get_losses(
                ...
                outputs, origin_preds, dtype=xin[0].dtype,
            )
        else:
    ...
    def get_losses(self, imgs, x_shifts, y_shifts, expanded_strides, labels, outputs, origin_preds, dtype):
        """
        Params:
            labels: (batch, labels_per_img, 5) for (class, cx,cy,w,h)
        """
            ...
            if num_gt == 0:
                ...
                try:
                    (
                        ...
                        x_shifts, y_shifts, cls_preds, bbox_preds, 
        ...
        loss_obj = self.obj_loss(
            obj_preds.flatten(), obj_targets.flatten(),
            alpha=0.9, gamma=2.0, reduction="sum",
        ) / num_fg
        loss_cls = (
            self.cls_loss(
        ...
        return 5.0 * loss_iou, loss_obj, loss_cls, loss_l1
    ...
    @torch.no_grad()
    def get_assignments(
        self, batch_idx, num_gt, total_num_anchors, gt_bboxes_per_image, gt_classes, bboxes_preds_per_image,
    ...
    def get_in_boxes_info(
        self, gt_bboxes_per_image, expanded_strides, 
        x_shifts, y_shifts, total_num_anchors, num_gt, img_size
    ):
